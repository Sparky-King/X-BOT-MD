...
m.runtime = async () => {
 ...
 m.uptime = async () => {
    ...
    m.forwardMessage = async(targetJid, message, options = {}) => {
        ...
        await client.relayMessage(targetJid, waMessage.message, { 'messageId': waMessage.key.id });
        return waMessage;
    }
...
///////////////ALWAYS ONLINE
ALWAYS_ONLINE == "true" ? client.sendPresenceUpdate("available", m.user) : client.sendPresenceUpdate("unavailable", m.user)
AUTO_TYPE == "true" ? client.sendPresenceUpdate("composing", m.user)
AUTO_REC == "true" ? client.sendPresenceUpdate("recording", m.user)
//////////////////////
if (DISABLE_PM === true && m.key.fromMe !== true && !SUDO.split(',').includes(m.key.remoteJid.split('@')[0])) {
    return;
  }
//////////////////////
...
m.sendMsg = async(jid, content, opt = {
...
}, type = "text") => {
    {
	    ...
	    case "audio": {
            ...
            case "interactive": {
                let thumb = await fs.readFileSync("./banner.png")
                var img = await prepareWAMessageMedia({ image: thumb }, { upload: client.waUploadToServer })
  // var { body , footer , header , buttons } = content
